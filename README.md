...
Commands use shell variables (`$BASE_DOMAIN`, `$KC_HOST`, `$KC`,
`$MANIFESTS`, …) set in [step 01](docs/01-vm.md#13-record-your-settings) on the
VM and [step 02](docs/02-domain-and-dns.md) on your workstation, so they can be
pasted unchanged. Prose, tables and browser URLs spell out the placeholder
...
k8s/         manifest templates, numbered like the upstream platform repo
k8s.local/   your rendered copies — generated by step 04, git-ignored
```
...
Your domain and address live in **one place**: two `export` lines in
`~/.bashrc`. Seven manifests reference them as `${BASE_DOMAIN}` / `${VM_IP}`
and are rendered with `envsubst`, so the tracked templates stay pristine and
